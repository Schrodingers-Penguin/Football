-- Performance: make the dashboard aggregate views MATERIALIZED so reads are
-- snapshot lookups instead of recomputing window functions / 62-column SUMs over
-- player_match_stats on every request (league page was ~6s, profile ~4s).
-- Refreshed after each rollup via refresh_dashboard_views(). Same view names, so
-- the app needs no changes.

DROP VIEW IF EXISTS player_season_percentiles CASCADE;
CREATE MATERIALIZED VIEW player_season_percentiles AS
SELECT
  pss.player_id,
  pss.season_id,
  pss.position_bucket,
...
CREATE UNIQUE INDEX player_season_percentiles_uidx ON player_season_percentiles (season_id, player_id, position_bucket);

DROP VIEW IF EXISTS team_season_stats CASCADE;
...
CREATE UNIQUE INDEX team_season_stats_uidx ON team_season_stats (season_id, team_id);

DROP VIEW IF EXISTS league_season_stats CASCADE;
...
CREATE UNIQUE INDEX league_season_stats_uidx ON league_season_stats (competition_id, season_id);

CREATE OR REPLACE FUNCTION refresh_dashboard_views() RETURNS void
LANGUAGE plpgsql AS $$
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY player_season_percentiles;
  REFRESH MATERIALIZED VIEW CONCURRENTLY team_season_stats;
  REFRESH MATERIALIZED VIEW CONCURRENTLY league_season_stats;
END;
$$;
